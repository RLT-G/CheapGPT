...
async def get_dialogue_history(id: int):
    async with async_session() as session:
        ...
        if user:
            ...
            dialog_title = str(user.dialog_titles).split(', ')[0:-1][int(current_dialog_index)]

            if str(current_dialog_index) not in message_history:
                return None

            message_history = message_history[str(current_dialog_index)]
            history = f'📋 История диалога "{dialog_title}"\n➖➖➖➖➖➖➖➖➖➖➖\n\n'
            for i, message in enumerate(message_history):
                emoji = '👤' if i % 2 == 0 else '🤖'
                endl = '\n' * 2 if i % 2 != 0 else '\n'
                history += f"{emoji}: {message.get('message')}{endl}"
...
async def get_referral_data(id: int):
    async with async_session() as session:
        ...
        if user:
            ...
            referral_link = rf'https://t.me/gpt_cheap_bot?start={id}'
